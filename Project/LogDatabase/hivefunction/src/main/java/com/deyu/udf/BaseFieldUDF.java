...
public class BaseFieldUDF extends UDF {
    ...
    public String evaluate(String line, String jsonKeys) {
        ...
        try {
            ...
            //2.切分jsonKeys
            String[] keysArray = jsonKeys.split(",");

            //3.原始数据进行切分
            String[] logContents = line.split("\\|");

            //4.取出时间
            String ts = logContents[0];

            //5.取出JSON串
            String jsonStr = logContents[1];
            ...
            //8.遍历keysArray，取出公共字段中的每一个值
            for (String key : keysArray) {

                //取出公共字段的Value
                String cmValue = cmJsonObj.getString(key);

                if (cmValue == null) {
                    sb.append("\t");
                } else {
                    sb.append(cmValue).append("\t");
                }
            }
